...
@pytest.mark.parametrize(
    "schema,schema_map,expected_result",
    [
        (None, None, "CREATE VIEW myview AS SELECT t1.col1, t1.col2 FROM t1"),
        (
            "myschema",
            None,
            "CREATE VIEW myschema.myview AS SELECT myschema.t1.col1, myschema.t1.col2 FROM myschema.t1",
        ),
    ],
)
...
def test_materialized_view():
    expected_result = """
    CREATE MATERIALIZED VIEW myview AS SELECT t1.col1, t1.col2 FROM t1
    """
    selectable = sa.sql.select([t1])
    view = Table("myview", sa.MetaData())
    create_view = CreateView(view, selectable, materialized=True)
...
def test_view_with_literals():
    ...
    view = Table("myview", sa.MetaData(), sa.Column("col3", sa.Integer()))
    create_view = CreateView(view, selectable)
    assert clean(expected_result) == clean(compile_query(create_view))
...
@pytest.mark.skipif(
    sqla_version < Version("1.4.0"), reason="Only for SQLAlchemy >= 1.4.0"
)
def test_view_with_on_parameter_is_not_none():
    ...
    with pytest.raises(TypeError):
        create_view = CreateView(view, selectable, on=True)  # on is not None


def test_view_with_bad_args():
    selectable = sa.sql.select([t1])
    view = Table("myview", sa.MetaData())
    create_view = CreateView(view, selectable, materialized=True, or_replace=True)
    with pytest.raises(ValueError):
        compile_query(create_view)
...
def test_drop_materialized():
    expected_result = """
    DROP MATERIALIZED VIEW myview
    """
    view = Table("myview", sa.MetaData())
    drop_view = DropView(view, materialized=True)
    assert clean(expected_result) == clean(compile_query(drop_view))
